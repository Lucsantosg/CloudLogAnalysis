...
In the {{site.data.keyword.cloud_notm}}, you can view, filter, and analyze Cloud Foundry (CF) logs through the {{site.data.keyword.cloud_notm}} dashboard, Kibana, and the command line interface. In addition, you can stream log records to an external log management tool. 
{:shortdesc}
...
{{site.data.keyword.cloud_notm}} records log data that is generated by the Cloud Foundry platform and by Cloud Foundry applications. In the logs, you can view the errors, warnings, and informational messages that are produced for your app. 

When you run your apps in a cloud platform-as-a-service (PaaS) like Cloud Foundry on the {{site.data.keyword.cloud_notm}}, you cannot SSH or FTP into the infrastructure where your apps are running to access the logs. The platform is controlled by the cloud provider. Cloud Foundry apps running on the {{site.data.keyword.cloud_notm}} use the Loggerator component to forward log records from inside of the Cloud Foundry infrastructure. The Loggregator automatically picks up STDOUT and STDERR data. You can visualize and analyze these logs through the {{site.data.keyword.cloud_notm}} dashboard, Kibana, and the command line interface.

The following figure shows a high level view of logging for Cloud Foundry apps in {{site.data.keyword.cloud_notm}}:

![High level component overview for CF apps](images/logging_cf_apps_ov.gif "High level component overview for CF apps")
 
Logging of Cloud Foundry apps is automatically enabled when you use the Cloud Foundry infrastructure to run your apps on the {{site.data.keyword.cloud_notm}}. To view Cloud Foundry runtime logs, you must write your logs to STDOUT and STDERR. 
...
By default, you can use Kibana to search up to 500 MB of logs per day in {{site.data.keyword.cloud_notm}}. 
...
* Analyze the log in the {{site.data.keyword.cloud_notm}} UI to view the latest activity of the application.
    
    In the {{site.data.keyword.cloud_notm}}, you can view, filter, and analyze logs through the **Log** tab that is available for each Cloud Foundry application. For more information, see [Analyzing CF app logs through the CF app dashboard](/docs/services/CloudLogAnalysis/cfapps?topic=cloudloganalysis-launch_logs_cloud_ui_cf#cfapp_ui).
    
* Analyze logs in Kibana to perform advanced analytical tasks.
    
    In the {{site.data.keyword.cloud_notm}}, you can use Kibana, an open source analytics and visualization platform, to monitor, search, analyze, and visualize your data in a variety of graphs, for example charts and tables. For more information, see [Analyzing CF app logs through the {{site.data.keyword.loganalysisshort}} UI](/docs/services/CloudLogAnalysis/cfapps?topic=cloudloganalysis-launch_logs_cloud_ui_cf#cfapp_la).
...
* Analyze logs through the CLI to use commands to manage logs programmatically.
    ...
    In the {{site.data.keyword.cloud_notm}}, you can view, filter, and analyze logs through the command line interface by using the **cf logs** command. For more information, see [Analyzing Cloud Foundry app logs from the command line interface](/docs/services/CloudLogAnalysis/cfapps?topic=cloudloganalysis-analyzing_logs_cli#analyzing_logs_cli).
...
## Log format for CF app logs viewed through the {{site.data.keyword.cloud_notm}} UI
{: #log_format_cf}

Logs for {{site.data.keyword.cloud_notm}} CF apps are displayed in a fixed format, similar to the following pattern:
