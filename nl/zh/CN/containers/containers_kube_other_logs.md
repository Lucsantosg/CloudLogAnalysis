...
只有具有 {{site.data.keyword.containershort}} 的 IAM 策略且具有管理集群许可权的用户才能启用此功能。需要以下任何角色：*管理员*、*操作员*

要检查用户标识已分配 IAM 策略来管理集群，请完成以下步骤：

**注：**只有具有分配策略许可权的帐户所有者或用户才可以执行此步骤。
...
3. 选择用户标识并验证该用户标识具有包含 {{site.data.keyword.containershort}} 的*查看者*许可权的策略。
...
3. 设置集群的终端上下文。
    ...
    运行此命令的输出提供您必须在终端运行的命令，以设置配置文件的路径。例如，对于名为 *MyCluster* 的集群：
...
例如，要创建将 stdout 和 stderr 日志转发到德国区域中的帐户的日志记录配置，请运行以下命令：
...
要创建将 stdout 和 stderr 日志转发到德国区域中的空间域的日志记录配置，请运行以下命令：
...
例如，要创建将应用程序日志转发到德国区域中的空间域的日志记录配置，请运行以下命令：
...
例如，要创建将应用程序日志转发到德国区域中的帐户域的日志记录配置，请运行以下命令：
...
例如，要创建将工作程序日志转发到德国区域中的空间域的日志记录配置，请运行以下命令：
...
例如，要创建将工作程序日志转发到德国区域中的帐户域的日志记录配置，请运行以下命令：
...
例如，要创建将 Kubernetes 系统组件日志转发到德国区域中的空间域的日志记录配置，请运行以下命令：
...
例如，要创建将 Kubernetes 系统组件日志转发到德国区域中的帐户域的日志记录配置，请运行以下命令：
...
例如，要创建将 Ingress 控制器日志转发到德国区域中的空间域的日志记录配置，请运行以下命令：
...
例如，要创建将 Ingress 控制器日志转发到德国区域中的帐户域的日志记录配置，请运行以下命令：
