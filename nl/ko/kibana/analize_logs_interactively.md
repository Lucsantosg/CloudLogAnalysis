...
| [인덱스화된 필드의 값으로 항목 순서 지정](/docs/services/CloudLogAnalysis/kibana/analize_logs_interactively.html#discover_sort_by_table) | 분석이 더 용이하도록 항목을 다시 정렬합니다. |
...
* **time:** 이 필드는 {{site.data.keyword.Bluemix_notm}}에서 항목이 캡처되고 기록되는 때를 표시합니다.
...
1. **관리** 페이지에서 인덱스 패턴을 확인하십시오.
    ...
    인덱스 패턴은 Kibana 페이지에 항목을 표시하도록 기본적으로 적용되는 검색 조회를 정의합니다. 기본적으로 인덱스 패턴은 사전 구성되고 {{site.data.keyword.Bluemix_notm}} 영역에서 사용 가능한 모든 데이터에 설정됩니다. 예: 

    * {{site.data.keyword.Bluemix_notm}} UI에서 Kibana를 실행하는 경우, 다시 말하면, Cloud Foundry(CF) 애플리케이션 또는 컨테이너와 같은 특정 리소스의 UI 페이지의 *로그* 섹션에서 Kibana를 실행하는 경우, 적용된 인덱스 패턴에는 영역에서 사용할 수 있는 모든 항목이 포함됩니다. 
...
2. 검색 페이지에서 조회를 확인하십시오.  
    ...
    * 검색 표시줄에 임의의 문자열을 입력하면 조회는 그 문자열에 대한 모든 필드를 스캔합니다.
...
3. 검색 페이지에서 조회에 적용된 필드 필터를 확인하십시오.
    ...
    필드 필터를 정의하여 필드의 값을 기반으로 항목을 전환할 수 있습니다. 예를 들어, 필드 필터를 사용할 수 있는 경우, 볼 수 있는 항목은 그 필드의 값이 일치하는 항목에 해당합니다.
    

## 인덱스화된 필드의 값으로 항목 순서 지정 
{: #discover_sort_by_table}

표에서 인덱스화된 필드에 대한 항목만 정렬할 수 있습니다.

무슨 필드가 인덱스화되었는지 찾아보려면 다음 단계를 완료하십시오.

1. 검색 페이지에서 그 페이지의 **사용 가능한 필드** 섹션에서 사용 가능한 구성 아이콘 ![구성 아이콘](images/configure_icon.jpg "구성 아이콘")을 클릭하십시오. 

2. 인덱스화된 필드를 식별하려면 검색 필드 **인덱스화됨**에 **예**를 선택하십시오.

    인덱스화된 필드의 목록이 표시됩니다.
 
인덱스화된 필드의 값으로 표에서 항목을 정렬하려면 다음 단계를 완료하십시오. 
...
예를 들어, 히스토그램에서 통계를 보려면 시간 소인을 클릭하여 그 기간에 대한 통계를 보십시오. 필드 목록에서 필드에 대한 통계를 보려면 이름을 클릭하십시오. 
