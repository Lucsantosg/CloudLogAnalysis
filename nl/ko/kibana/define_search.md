...
검색 페이지의 검색 표시줄에서 Lucene 조회 언어를 사용하여 검색 쿼리를 정의하고 저장할 수 있습니다. 각 검색에 대해 필터를 적용하여 분석에 사용할 수 있는 항목을 세분화할 수 있습니다.
{:shortdesc}
...
1. Kibana를 실행하십시오. 
    ...
    Cloud Foundry(CF) 앱의 경우, [CF 앱의 대시보드에서 Kibana 실행](/docs/services/CloudLogAnalysis/kibana/launch.html#launch_Kibana_from_cf_app)을 참조하십시오.

	{{site.data.keyword.IBM_notm}} 관리 클라우드 인프라에서 실행되는 컨테이너의 경우, [컨테이너의 대시보드에서 Kibana 실행](/docs/services/CloudLogAnalysis/kibana/launch.html#launch_Kibana_for_containers)을 참조하십시오.
	...
	Kibana에 액세스하면 기본 검색이 적용됩니다. 조회 중인 리소스에 해당하는 인스턴스의 목록에 대한 로그를 볼 수 있습니다. 그 영역에서 임의의 또는 모든 {{site.data.keyword.Bluemix_notm}} 리소스에 대한 로그를 필터링할 수 있습니다.

2. 검색 페이지에 어떤 데이터의 서브세트가 표시되는지 보십시오. 자세한 정보는 [Kibana 검색 페이지에 표시된 데이터 식별](/docs/services/CloudLogAnalysis/kibana/analize_logs_interactively.html#identify_data)을 참조하십시오. 그 다음에 항목을 필터링하기 위한 기본 조회를 수정하십시오.

    **참고:** Lucene 조회 언어를 사용하여 사용자 정의 조회를 정의합니다. 자세한 정보는 [Apache Lucene - 조회 구문 분석기 구문 ![외부 링크 아이콘](../../../icons/launch-glyph.svg "외부 링크 아이콘")](https://lucene.apache.org/core/2_9_4/queryparsersyntax.html "외부 링크 아이콘"){: new_window}을 참조하십시오.
    
    {{site.data.keyword.Bluemix_notm}}에서 Kibana가 실행되면 조회를 수정하고 다중 검색 기준을 정의하기 위해 논리 용어 **AND** 및 **OR**를 사용할 수 있습니다. 이러한 연산자는 대문자여야 합니다.    
