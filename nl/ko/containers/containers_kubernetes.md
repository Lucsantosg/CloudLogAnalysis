...
{{site.data.keyword.cloud_notm}}에서 {{site.data.keyword.loganalysisshort}} 서비스를 사용하여 {{site.data.keyword.containershort}}가 공용 및 전용으로 자동으로 수집하는 컨테이너 로그 및 Kubernetes 클러스터 로그를 저장하고 분석할 수 있습니다.
{:shortdesc}

계정에 1개 이상의 Kubernetes 클러스터가 있을 수 있습니다. 클러스터가 프로비저닝되는 즉시 {{site.data.keyword.containershort}}가 자동으로 로그를 수집합니다. 

{{site.data.keyword.containershort}}에서 {{site.data.keyword.loganalysisshort}}로 로그를 전달하려면 로깅 구성을 작성해야 합니다. [명령행](/docs/containers/cs_health.html#logging) 또는 {{site.data.keyword.containershort}} 대시보드의 클러스터 개요 페이지를 통해 로깅을 사용할 수 있습니다. 구성이 있으면 클러스터를 프로비저닝할 때 또는 팟(Pod)을 배치하는 즉시 {{site.data.keyword.containershort}}에서 로그를 수집합니다. 컨테이너에서 `stdout` 또는 `stderr`에 인쇄하는 정보도 자동으로 수집됩니다. 기본적으로 로그는 클러스터의 위치를 기반으로 특정 지역으로 전달됩니다.
...
클러스터가 배치된 위치에 따라 {{site.data.keyword.loganalysisshort}}에서 로그를 보내는 지역을 보려면 다음 표를 확인하십시오.
 * {{site.data.keyword.containershort}}는 컨테이너 프로세스가 stdout(표준 출력) 및 stderr(표준 오류)로 인쇄하는 정보를 자동으로 수집합니다.
 ...
 로그를 보려고 할 때 올바른 지역, 조직 및 영역을 대상으로 하십시오. {{site.data.keyword.loganalysisshort}} GUI를 통해 올바른 조직과 영역을 대상으로 지정하는지도 확인할 수 있습니다.

 {: tip}

로그를 {{site.data.keyword.loganalysisshort}} 계정 또는 계정의 영역 도메인으로 전달할 수 있습니다. 로그를 전달할 위치를 결정할 때 다음 정보를 고려하십시오.
...
로그를 {{site.data.keyword.loganalysisshort}}로 전달하도록 클러스터를 구성하는 경우 다음 정보를 고려하십시오.

* 해당 로그가 전달될 Cloud Foundry 조직 및 영역을 정의해야 합니다. 
* 조직 및 영역은 모든 {{site.data.keyword.IBM_notm}} Public Cloud 지역에서 사용 가능합니다.

**참고:** **{{site.data.keyword.cloud_notm}} 데디케이티드**에 프로비저닝된 클러스터의 경우 클러스터를 전용 계정에서 사용 가능한 Cloud Foundry 영역으로 전달하도록 클러스터를 구성할 수 없습니다.
...
* 사용자 ID에 로그를 볼 수 있는 권한이 있어야 합니다. 영역 도메인에서 로그를 보려면 사용자에게 CF 역할이 필요합니다. **감사자**는 로그를 보기 위해 부여할 수 있는 최하위 역할입니다. 자세한 정보는 [로그 보기를 위해 사용자에게 필요한 역할](/docs/services/CloudLogAnalysis/kibana?topic=cloudloganalysis-analyzing_logs_Kibana#roles)을 참조하십시오.

장기 스토리지(로그 콜렉션)에 저장된 클러스터 로그 데이터를 관리하려면 사용자 ID에 {{site.data.keyword.loganalysisshort}} 서비스에 대한 작업을 수행하기 위한 IAM 정책이 있어야 합니다. 사용자 ID에 **관리자**, **운영자** 또는 **편집자** 권한이 있어야 합니다.  자세한 정보는 [로그 관리를 위해 사용자에게 필요한 역할](/docs/services/CloudLogAnalysis?topic=cloudloganalysis-manage_logs#roles1)을 참조하십시오.
...
계정이 있는 환경({{site.data.keyword.cloud_notm}} 퍼블릭 또는 데디케이티드 환경)에 상관없이 계정 도메인에 로그를 전달하도록 클러스터를 구성할 수 있습니다.
{: shortdesc}


로그를 계정 도메인으로 전달하는 Kibana에서 클러스터에 대한 로그 데이터를 분석하려면 다음 정보를 고려하십시오.

* 클러스터가 {{site.data.keyword.loganalysisshort}} 서비스로 로그를 보내는 지역에서 Kibana를 실행해야 합니다.
* 로그 콜렉션의 장기 스토리지에 저장된 클러스터 로그 데이터를 관리하려면 {{site.data.keyword.loganalysisshort}} 서비스에 대한 작업을 수행하기 위한 IAM 역할이 있어야 합니다. 사용자 ID에 **관리자**, **운영자** 또는 **편집자** 권한이 있어야 합니다. 로그를 보려면 **뷰어** 권한이 있어야 합니다.


다음 이미지에서는 공용 환경에서 계정에 로그 전달이 작동하는 방식을 대략적으로 볼 수 있습니다.

![Kubernetes 클러스터에 배치된 컨테이너의 상위 레벨 컴포넌트 개요](images/containers_kube.png "Kubernetes 클러스터에 배치된 컨테이너의 상위 레벨 컴포넌트 개요")

다음 이미지에서는 데디케이티드 환경에서 계정에 로그 전달이 작동하는 방식을 대략적으로 볼 수 있습니다.
...
로그 파일을 {{site.data.keyword.loganalysisshort}} 서비스로 전달하도록 클러스터를 구성하는 방법에 대한 자세한 정보는 [클러스터 로그의 자동 수집 사용](/docs/services/CloudLogAnalysis/containers?topic=cloudloganalysis-containers_kube_other_logs#containers_kube_other_logs) 섹션을 참조하십시오.
...
## {{site.data.keyword.cloud_notm}}에서 사용자 정의 방화벽 구성에 대한 네트워크 트래픽 구성
{: #ports}

추가 방화벽이 설치되었거나 {{site.data.keyword.cloud_notm}} 인프라(SoftLayer)에서 방화벽 설정을 사용자 정의한 경우 작업자 노드에서 {{site.data.keyword.loganalysisshort}} 서비스로 발신 네트워크 트래픽을 허용해야 합니다.
...
기본적으로 Kibana를 사용하여 {{site.data.keyword.cloud_notm}}에서 하루에 최대 500MB의 로그를 검색할 수 있습니다. 

더 큰 로그를 검색하기 위해 {{site.data.keyword.loganalysisshort}} 서비스를 사용할 수 있습니다. 서비스는 여러 가지 플랜을 제공합니다. 각 플랜에는 각기 다른 로그 검색 기능이 있으며, 예를 들어 *로그 콜렉션* 플랜은 하루에 최대 1GB의 데이터를 검색하도록 허용합니다. 사용 가능한 플랜에 대한 자세한 정보는 [서비스 플랜](/docs/services/CloudLogAnalysis?topic=cloudloganalysis-log_analysis_ov#plans)을 참조하십시오.
...
기본적으로 로깅은 자동으로 컨테이너에 사용 가능하게 설정됩니다. Docker 로그 파일의 모든 항목은 Kibana에서 **message** 필드에 표시됩니다. 컨테이너 로그 항목의 일부인 특정 필드를 사용하여 Kibana의 데이터를 필터링하고 분석해야 하는 경우, 올바른 JSON 형식화된 출력을 전송하도록 애플리케이션을 구성하십시오. 예를 들어 JSON 형식의 메시지를 `stdout` 및 `stderr`에 로깅하십시오.
 
메시지에서 사용 가능한 각 필드는 값과 일치하는 필드의 유형으로 구문 분석됩니다.
 
예를 들어 다음 JSON 메시지의 각 필드는 필터링과 검색에 사용할 수 있는 필드로 사용 가능합니다.
...
클러스터 로그를 {{site.data.keyword.loganalysisshort}}로 전달하려면, {{site.data.keyword.cloud_notm}} 권한을 {{site.data.keyword.containershort}} 키 소유자 및 로깅 클러스터 구성을 구성하는 사용자 ID에 부여해야 합니다.
...
로그에 대해 작업할 때 {{site.data.keyword.cloud_notm}}의 기본 동작에 대한 다음 정보를 고려하십시오.

* 로그 데이터는 최대 3일 동안 저장됩니다.
...
* 로그 콜렉션의 로그를 관리하려면 사용자에게 {{site.data.keyword.cloud_notm}}에서 {{site.data.keyword.loganalysisshort}} 서비스에 대해 작업하기 위한 권한이 있는 IAM 정책이 필요합니다. 자세한 정보는 [IAM 역할](/docs/services/CloudLogAnalysis?topic=cloudloganalysis-security_ov#iam_roles)을 참조하십시오.

## 로그 보기 및 분석
{: #logging_containers_ov_methods}

로그 데이터를 분석하려면 Kibana를 사용하여 고급 분석 태스크를 수행하십시오. Kibana는 다양한 그래프(예: 차트 및 표)로 된 데이터를 모니터, 검색, 분석 및 시각화하는 데 사용할 수 있는 오픈 소스의 분석 및 시각화 플랫폼입니다. 자세한 정보는 [Kibana에서 로그 분석](/docs/services/CloudLogAnalysis/kibana?topic=cloudloganalysis-analyzing_logs_Kibana#analyzing_logs_Kibana)을 참조하십시오.
...
* 클러스터 컨텍스트 내의 {{site.data.keyword.cloud_notm}} UI에서 Kibana를 실행할 수 있습니다. 자세한 정보는 [Kubernetes 클러스터에 배치된 컨테이너의 대시보드에서 Kibana로 이동](/docs/services/CloudLogAnalysis/kibana?topic=cloudloganalysis-launch#launch_Kibana_for_containers_kube)을 참조하십시오.

컨테이너에서 실행되는 앱의 로그 데이터를 JSON 형식으로 컨테이너 런타임 로그 콜렉터에 전달하는 경우, JSON 필드를 사용하여 Kibana에서 로그 데이터를 검색하고 분석할 수 있습니다. 자세한 정보는 [메시지의 필드를 Kibana 검색 필드로 사용할 수 있도록 로그 전송](/docs/services/CloudLogAnalysis/containers?topic=cloudloganalysis-containers_kubernetes#send_data_in_json)을 참조하십시오.
...
Kibana를 사용하여 Kubernetes 클러스터에 배치된 컨테이너의 로그를 분석하는 방법을 알아보려면 [Kibana에서 Kubernetes 클러스터에 배치된 앱에 대한 로그 분석](/docs/services/CloudLogAnalysis/tutorials?topic=cloudloganalysis-container_logs#container_logs)을 참조하십시오.
