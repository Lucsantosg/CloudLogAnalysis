...
2. `bx cf services` 명령을 실행하여 현재 플랜을 확인하고 영역에서 사용할 수 있는 서비스 목록에서 {{site.data.keyword.loganalysisshort}} 서비스 이름을 가져오십시오.  
...
3. 플랜을 업그레이드하거나 줄이십시오. `bx cf update-service` 명령을 실행하십시오. 
	...
	* *service_name*은 서비스의 이름입니다. `bx cf services` 명령을 실행하여 값을 가져올 수 있습니다. 
